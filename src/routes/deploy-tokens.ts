...
function generateDeployToken(repoId: string, name: string): string {
  const payload = {
    type: 'deploy',
    repoId,
    name,
...
// Verify deploy token
export function verifyDeployToken(token: string): any {
  try {
...
// Create deploy token
router.post('/:owner/:repo/tokens', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    await dbRun(
      `INSERT INTO deploy_tokens (id, repository_id, token, name, permissions, expires_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [tokenId, repository.id, token, name, perms, expiresAt]
    );
...
router.get('/:owner/:repo/tokens', authenticate, async (req: AuthRequest, res) => {
  try {
    const { owner, repo } = req.params;
    ...
    const tokens = await dbAll(
      'SELECT id, name, permissions, expires_at, created_at, last_used_at FROM deploy_tokens WHERE repository_id = ? ORDER BY created_at DESC',
      [repository.id]
    );
...
// Delete deploy token
router.delete('/:owner/:repo/tokens/:tokenId', authenticate, async (req: AuthRequest, res) => {
  try {
