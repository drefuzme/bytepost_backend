...
async function readRepoFiles(repoPath: string, branch: string = 'main', basePath: string = ''): Promise<any[]> {
  const files: any[] = [];
...
router.post('/live-server/start-from-repo', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (!isOwner && !isPublic) {
      const collaborator: any = await dbGet(
    ...
    // Find HTML file
    let htmlFile = files.find((f: any) => f.path === htmlPath);
    if (!htmlFile) {
      htmlFile = files.find((f: any) => f.path.endsWith('.html') || f.path.endsWith('.htm'));
    }
    if (!htmlFile) {
    ...
    const httpServer = createServer(async (req, res) => {
      try {
        ...
        try {
          ...
          else if (filePath.endsWith('.woff') || filePath.endsWith('.woff2')) contentType = 'font/woff';
...
// Start live server
router.post('/live-server/start', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    // Find HTML file
    const htmlFile = files.find((f: any) => f.path.endsWith('.html') || f.path.endsWith('.htm'));
    if (!htmlFile) {
      return res.status(400).json({ error: 'HTML fayl topilmadi' });
    }
...
// Stop live server
router.post('/live-server/stop', authenticate, async (req: AuthRequest, res) => {
  try {
...
router.get('/live-server/active', authenticate, async (req: AuthRequest, res) => {
  try {
    const userId = req.user!.userId;
