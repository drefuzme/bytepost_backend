...
router.get('/me', authenticate, async (req: AuthRequest, res) => {
  try {
    const user: any = await dbGet('SELECT id, username, email, avatar_url, role, is_verified, icon_type, verify_icon_type, bio, created_at FROM users WHERE id = ?', 
      [req.user!.userId]);
...
router.get('/recommendations', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const allUsersCount: any = await dbGet('SELECT COUNT(*) as count FROM users WHERE id != ?', [req.user!.userId]);
    console.log('All users count (excluding self):', allUsersCount?.count || 0);

    // Get users that current user is following
    const followingUsers: any = await dbAll(
    ...
    if (getAll) {
      ...
      const followingIds = followingUsers.map((f: any) => f.following_id);
...
router.get('/:username', optionalAuthenticate, async (req: AuthRequest, res) => {
  try {
    const { username } = req.params;
    const userId = req.user?.userId || null;

    const user: any = await dbGet(
      'SELECT id, username, email, avatar_url, is_verified, icon_type, verify_icon_type, bio, created_at FROM users WHERE username = ?',
      [username]
    );
    ...
    const followersCount: any = await dbGet(
      'SELECT COUNT(*) as count FROM user_follows WHERE following_id = ?',
      [user.id]
    );

    const followingCount: any = await dbGet(
    ...
    if (userId) {
      const followCheck: any = await dbGet(
...
// Update user bio
router.patch('/me/bio', authenticate, async (req: AuthRequest, res) => {
  try {
    const { bio } = req.body;
    ...
    await dbRun(
      ...
      [bio || null, req.user!.userId]
    );

    const updatedUser: any = await dbGet(
      'SELECT id, username, email, avatar_url, role, is_verified, icon_type, verify_icon_type, bio, created_at FROM users WHERE id = ?',
      [req.user!.userId]
    );
...
router.patch('/me', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const existingUser: any = await dbGet(
    ...
    const existingEmail: any = await dbGet(
...
router.post('/:username/follow', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const targetUser: any = await dbGet(
    ...
    const existingFollow: any = await dbGet(
      'SELECT * FROM user_follows WHERE follower_id = ? AND following_id = ?',
    ...
    if (existingFollow) {
      ...
      await dbRun(
        ...
        [followId, req.user!.userId, targetUser.id]
      );
