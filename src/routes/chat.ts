...
router.get('/conversations', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    for (const conv of conversations) {
      ...
      conv.participants = allParticipants.filter((p: any) => p.id !== req.user!.userId);
      conv.all_participants = allParticipants;
      
      // Get current user's role
      const currentUserParticipant = allParticipants.find((p: any) => p.id === req.user!.userId);
  ...
  try {
    ...
    if (type === 'group') {
      ...
      await dbRun(
        ...
        [conversationId, 'group', name.trim(), req.user!.userId]
      );
      ...
      await dbRun(
        ...
        [creatorParticipantId, conversationId, req.user!.userId, 'moderator']
      );
      ...
      for (const userId of userIds) {
        if (userId !== req.user!.userId) {
          ...
          await dbRun(
            'INSERT INTO conversation_participants (id, conversation_id, user_id, role) VALUES (?, ?, ?, ?)',
            [participantId, conversationId, userId, 'member']
          );
        }
      }
    ...
    await dbRun(
      ...
      [participant1Id, conversationId, req.user!.userId]
    );
    await dbRun(
      'INSERT INTO conversation_participants (id, conversation_id, user_id) VALUES (?, ?, ?)',
      [participant2Id, conversationId, userId]
    );
...
router.get('/conversations/:id/messages', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const participant: any = await dbGet(
...
router.post('/conversations/:id/messages', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    try {
      ...
      try {
        await dbRun(
          'INSERT INTO messages (id, conversation_id, sender_id, content, image_url) VALUES (?, ?, ?, ?, ?)',
          [messageId, id, req.user!.userId, messageContent, image_url || null]
        );
        ...
        if (insertErr.message?.includes('no such column: image_url') || insertErr.message?.includes('SQLITE_ERROR')) {
          ...
          await dbRun(
            ...
            [messageId, id, req.user!.userId, messageContent]
          );
...
router.get('/conversations/:id', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    try {
      ...
      conversation = await dbGet(
        'SELECT id, type, NULL as name, NULL as created_by, created_at, updated_at FROM conversations WHERE id = ?',
        [id]
      );
    }
    ...
    // Get current user's role
    const currentUserParticipant = participants.find((p: any) => p.id === req.user!.userId);
...
// Add user to group
router.post('/conversations/:id/participants', authenticate, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    ...
    const conversation: any = await dbGet('SELECT type, created_by FROM conversations WHERE id = ?', [id]);
    if (!conversation) {
    ...
    await dbRun(
      'INSERT INTO conversation_participants (id, conversation_id, user_id, role) VALUES (?, ?, ?, ?)',
      [participantId, id, userId, 'member']
    );
...
router.patch('/conversations/:id', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const updatedConversation: any = await dbGet(
      'SELECT id, type, name, created_by, created_at, updated_at FROM conversations WHERE id = ?',
      [id]
    );
...
router.delete('/conversations/:id/participants/:userId', authenticate, async (req: AuthRequest, res) => {
  try {
