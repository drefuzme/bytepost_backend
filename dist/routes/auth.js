...
router.post('/register', async (req, res) => {
    try {
        ...
        await dbRun('INSERT INTO users (id, username, email, password, role, icon_type) VALUES (?, ?, ?, ?, ?, ?)', [userId, username, email, hashedPassword, userRole, 'user']);
        // Generate token
...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        await dbRun('DELETE FROM password_reset_tokens WHERE user_id = ? AND used = 0', [user.id]);
        // Save reset token
        await dbRun('INSERT INTO password_reset_tokens (id, user_id, token, expires_at) VALUES (?, ?, ?, ?)', [tokenId, user.id, resetToken, expiresAt.toISOString()]);
        // Send email
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        // Find reset token
        const resetToken = await dbGet('SELECT * FROM password_reset_tokens WHERE token = ? AND used = 0', [token]);
        if (!resetToken) {
