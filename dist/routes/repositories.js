...
router.get('/:owner/:repo', optionalAuthenticate, async (req, res) => {
    try {
    ...
    `, [userId, owner, repo]);
        ...
        if (repository.is_private) {
            ...
            const hasAccess = repository.owner_id === req.user.userId ||
                await dbGet('SELECT * FROM repository_collaborators WHERE repository_id = ? AND user_id = ?', [repository.id, req.user.userId]);
            if (!hasAccess) {
...
router.post('/', authenticate, async (req, res) => {
    try {
        ...
        const existing = await dbGet('SELECT * FROM repositories WHERE name = ? AND owner_id = ?', [name, req.user.userId]);
        if (existing) {
        ...
        await dbRun(`INSERT INTO repositories (id, name, description, owner_id, is_private, default_branch)
       VALUES (?, ?, ?, ?, ?, ?)`, [repoId, name, description || '', req.user.userId, isPrivate ? 1 : 0, 'main']);
...
// Update repository
router.patch('/:owner/:repo', authenticate, async (req, res) => {
    try {
        const { owner, repo } = req.params;
        const { name, description, isPrivate } = req.body;
        const repository = await dbGet(`
    ...
    `, [owner, repo]);
        ...
        if (name && name !== repo) {
            const existing = await dbGet('SELECT * FROM repositories WHERE name = ? AND owner_id = ? AND id != ?', [name, repository.owner_id, repository.id]);
            if (existing) {
                return res.status(400).json({ error: 'Bu nom bilan repository allaqachon mavjud' });
            }
        }
...
// Delete repository
router.delete('/:owner/:repo', authenticate, async (req, res) => {
    try {
        const { owner, repo } = req.params;
        const repository = await dbGet(`
      SELECT r.* FROM repositories r
...
router.post('/:owner/:repo/star', authenticate, async (req, res) => {
    try {
        const { owner, repo } = req.params;
        const repository = await dbGet(`
      SELECT r.*, u.username as owner_username
    ...
    `, [owner, repo]);
        ...
        const existingStar = await dbGet('SELECT * FROM repository_stars WHERE repository_id = ? AND user_id = ?', [repository.id, req.user.userId]);
        if (existingStar) {
            // Unstar
            ...
            const starsCount = await dbGet('SELECT COUNT(*) as count FROM repository_stars WHERE repository_id = ?', [repository.id]);
...
router.get('/:owner/:repo/collaborators', authenticate, async (req, res) => {
    try {
        const { owner, repo } = req.params;
        const repository = await dbGet(`
      SELECT r.* FROM repositories r
...
router.post('/:owner/:repo/collaborators', authenticate, async (req, res) => {
    try {
        const { owner, repo } = req.params;
    ...
    `, [owner, repo]);
        ...
        const userToAdd = await dbGet('SELECT * FROM users WHERE username = ?', [username]);
        if (!userToAdd) {
        ...
        const existing = await dbGet('SELECT * FROM repository_collaborators WHERE repository_id = ? AND user_id = ?', [repository.id, userToAdd.id]);
        if (existing) {
            return res.status(400).json({ error: 'Bu foydalanuvchi allaqachon collaborator' });
        }
        // Add collaborator
        const collaboratorId = uuidv4();
        await dbRun(`INSERT INTO repository_collaborators (id, repository_id, user_id, permission)
       VALUES (?, ?, ?, ?)`, [collaboratorId, repository.id, userToAdd.id, permission]);
        // Get owner info for notification and chat
        const ownerInfo = await dbGet('SELECT username, id FROM users WHERE id = ?', [repository.owner_id]);
        ...
        try {
            const notificationId = await createNotification(userToAdd.id, 'collaborator_invite', `Sizga repository'ga collaborator sifatida taklif yuborildi`, `${ownerInfo?.username || 'Foydalanuvchi'} sizni "${repository.name}" repository'siga collaborator sifatida qo'shdi. Endi siz bu repository'ga push qilish va fayllarni edit qilish huquqiga egasiz.`, `/${ownerInfo?.username || ''}/${repository.name}`);
      ...
      `, [repository.owner_id, userToAdd.id]);
            if (existingConv) {
                ...
                await dbRun('INSERT INTO conversation_participants (id, conversation_id, user_id) VALUES (?, ?, ?)', [participant1Id, conversationId, repository.owner_id]);
                await dbRun('INSERT INTO conversation_participants (id, conversation_id, user_id) VALUES (?, ?, ?)', [participant2Id, conversationId, userToAdd.id]);
            ...
            await dbRun('INSERT INTO messages (id, conversation_id, sender_id, content) VALUES (?, ?, ?, ?)', [messageId, conversationId, repository.owner_id, welcomeMessage]);
...
router.delete('/:owner/:repo/collaborators/:userId', authenticate, async (req, res) => {
    try {
    ...
    `, [owner, repo]);
        ...
        // Remove collaborator
        await dbRun('DELETE FROM repository_collaborators WHERE repository_id = ? AND user_id = ?', [repository.id, userId]);
...
router.get('/activity/issues', optionalAuthenticate, async (req, res) => {
    try {
...
router.get('/activity/pulls', optionalAuthenticate, async (req, res) => {
    try {
        const userId = req.user?.userId;
        let query = `
      SELECT 
...
router.get('/activity/commits', optionalAuthenticate, async (req, res) => {
    try {
