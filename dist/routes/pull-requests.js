...
router.get('/:owner/:repo/pulls', optionalAuthenticate, async (req, res) => {
    try {
...
router.get('/:owner/:repo/pulls/:number', optionalAuthenticate, async (req, res) => {
    try {
...
// Create pull request
router.post('/:owner/:repo/pulls', authenticate, async (req, res) => {
    try {
        const { owner, repo } = req.params;
    ...
    `, [
        ...
        if (repository.owner_id !== req.user.userId) {
            await createNotification(repository.owner_id, 'pull_request_created', `New pull request in ${owner}/${repo}`, `${req.user.username} opened pull request #${nextNumber}: ${title.trim()}`, `/${owner}/${repo}/pulls/${nextNumber}`);
        }
        ...
        if (labels && Array.isArray(labels)) {
            for (const labelId of labels) {
                ...
                if (labelExists) {
                    await dbRun('INSERT INTO pull_request_labels (id, pull_request_id, label_id) VALUES (?, ?, ?)', [uuidv4(), prId, labelId]);
                }
            }
        }
        const pr = await dbGet(`
...
// Update pull request
router.patch('/:owner/:repo/pulls/:number', authenticate, async (req, res) => {
    try {
        const { owner, repo, number } = req.params;
    ...
    `, params);
        ...
        if (labels !== undefined && Array.isArray(labels)) {
            ...
            for (const labelId of labels) {
                const labelExists = await dbGet('SELECT * FROM labels WHERE id = ? AND repository_id = ?', [labelId, repository.id]);
                if (labelExists) {
                    await dbRun('INSERT INTO pull_request_labels (id, pull_request_id, label_id) VALUES (?, ?, ?)', [uuidv4(), pr.id, labelId]);
                }
            }
        }
        const updatedPR = await dbGet(`
      SELECT 
        pr.*,
        u.username as author_username,
        u.avatar_url as author_avatar
...
router.post('/:owner/:repo/pulls/:number/comments', authenticate, async (req, res) => {
    try {
        const { owner, repo, number } = req.params;
        const { body } = req.body;
        if (!body || !body.trim()) {
    ...
    `, [commentId, pr.id, req.user.userId, body.trim()]);
        ...
        if (pr.author_id !== req.user.userId) {
            await createNotification(pr.author_id, 'pull_request_comment', `New comment on pull request #${pr.number}`, `${req.user.username} commented on your pull request: ${pr.title}`, `/${owner}/${repo}/pulls/${pr.number}`);
        }
        ...
        if (repository.owner_id !== req.user.userId && repository.owner_id !== pr.author_id) {
            await createNotification(repository.owner_id, 'pull_request_comment', `New comment on pull request #${pr.number}`, `${req.user.username} commented on pull request #${pr.number} in ${owner}/${repo}`, `/${owner}/${repo}/pulls/${pr.number}`);
        }
