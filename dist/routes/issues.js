...
router.get('/:owner/:repo/issues', optionalAuthenticate, async (req, res) => {
    try {
...
// Get single issue
router.get('/:owner/:repo/issues/:number', optionalAuthenticate, async (req, res) => {
    try {
...
// Create issue
router.post('/:owner/:repo/issues', authenticate, async (req, res) => {
    try {
        const { owner, repo } = req.params;
    ...
    `, [issueId, repository.id, nextNumber, title.trim(), body || '', req.user.userId]);
        ...
        if (repository.owner_id !== req.user.userId) {
            await createNotification(repository.owner_id, 'issue_created', `New issue in ${owner}/${repo}`, `${req.user.username} opened issue #${nextNumber}: ${title.trim()}`, `/${owner}/${repo}/issues/${nextNumber}`);
        }
        ...
        if (labels && Array.isArray(labels)) {
            for (const labelId of labels) {
                ...
                if (labelExists) {
                    await dbRun('INSERT INTO issue_labels (id, issue_id, label_id) VALUES (?, ?, ?)', [uuidv4(), issueId, labelId]);
                }
            }
        }
        const issue = await dbGet(`
...
// Update issue
router.patch('/:owner/:repo/issues/:number', authenticate, async (req, res) => {
    try {
        const { owner, repo, number } = req.params;
    ...
    `, params);
        ...
        if (labels !== undefined && Array.isArray(labels)) {
            ...
            for (const labelId of labels) {
                const labelExists = await dbGet('SELECT * FROM labels WHERE id = ? AND repository_id = ?', [labelId, repository.id]);
                if (labelExists) {
                    await dbRun('INSERT INTO issue_labels (id, issue_id, label_id) VALUES (?, ?, ?)', [uuidv4(), issue.id, labelId]);
                }
            }
        }
        const updatedIssue = await dbGet(`
      SELECT 
        i.*,
        u1.username as author_username,
        u1.avatar_url as author_avatar,
...
router.post('/:owner/:repo/issues/:number/comments', authenticate, async (req, res) => {
    try {
        const { owner, repo, number } = req.params;
        const { body } = req.body;
        if (!body || !body.trim()) {
    ...
    `, [commentId, issue.id, req.user.userId, body.trim()]);
        ...
        if (issue.author_id !== req.user.userId) {
            await createNotification(issue.author_id, 'issue_comment', `New comment on issue #${issue.number}`, `${req.user.username} commented on your issue: ${issue.title}`, `/${owner}/${repo}/issues/${issue.number}`);
        }
        ...
        if (repository.owner_id !== req.user.userId && repository.owner_id !== issue.author_id) {
            await createNotification(repository.owner_id, 'issue_comment', `New comment on issue #${issue.number}`, `${req.user.username} commented on issue #${issue.number} in ${owner}/${repo}`, `/${owner}/${repo}/issues/${issue.number}`);
        }
...
router.get('/:owner/:repo/labels', optionalAuthenticate, async (req, res) => {
    try {
        const { owner, repo } = req.params;
        const repository = await dbGet(`
      SELECT r.* FROM repositories r
...
// Create label
router.post('/:owner/:repo/labels', authenticate, async (req, res) => {
    try {
        const { owner, repo } = req.params;
