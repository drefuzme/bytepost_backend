...
router.post('/', authenticate, async (req, res) => {
    try {
        ...
        const postId = uuidv4();
        await dbRun(`INSERT INTO posts (id, user_id, content, image_url) VALUES (?, ?, ?, ?)`, [postId, req.user.userId, content.trim(), imageUrl || null]);
        const post = await dbGet(`
      SELECT 
        p.*,
        u.username,
...
router.patch('/:id', authenticate, async (req, res) => {
    try {
        ...
        await dbRun('UPDATE posts SET content = ?, image_url = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [content.trim(), imageUrl || null, id]);
...
router.post('/:id/like', authenticate, async (req, res) => {
    try {
        ...
        const existingLike = await dbGet('SELECT * FROM post_likes WHERE post_id = ? AND user_id = ?', [id, req.user.userId]);
        if (existingLike) {
            // Unlike
            ...
            if (post.user_id !== req.user.userId) {
                await createNotification(post.user_id, 'post_like', 'New like on your post', `${req.user.username} liked your post`, `/blog/post/${id}`);
            }
            res.json({ liked: true });
        }
    }
...
router.post('/:id/repost', authenticate, async (req, res) => {
    try {
        ...
        const existingRepost = await dbGet('SELECT * FROM post_reposts WHERE post_id = ? AND user_id = ?', [id, req.user.userId]);
        if (existingRepost) {
            // Unrepost
            ...
            if (post.user_id !== req.user.userId) {
                await createNotification(post.user_id, 'post_repost', 'New repost of your post', `${req.user.username} reposted your post`, `/blog/post/${id}`);
            }
...
router.post('/:id/comments', authenticate, async (req, res) => {
    try {
        ...
        await dbRun('INSERT INTO comments (id, post_id, user_id, content) VALUES (?, ?, ?, ?)', [commentId, id, req.user.userId, content.trim()]);
        ...
        if (post.user_id !== req.user.userId) {
            await createNotification(post.user_id, 'post_comment', 'New comment on your post', `${req.user.username} commented on your post`, `/blog/post/${id}`);
        }
...
router.post('/:id/poll/vote', authenticate, async (req, res) => {
    try {
        ...
        const existingVote = await dbGet('SELECT * FROM poll_votes WHERE post_id = ? AND user_id = ?', [id, req.user.userId]);
        if (existingVote) {
            // Update vote
            await dbRun('UPDATE poll_votes SET option_index = ? WHERE post_id = ? AND user_id = ?', [optionIndex, id, req.user.userId]);
        }
        else {
            // Create new vote
            const voteId = uuidv4();
            await dbRun('INSERT INTO poll_votes (id, post_id, user_id, option_index) VALUES (?, ?, ?, ?)', [voteId, id, req.user.userId, optionIndex]);
        }
        // Get poll results
        const votes = await dbAll('SELECT option_index, COUNT(*) as count FROM poll_votes WHERE post_id = ? GROUP BY option_index', [id]);
...
// Get poll results
router.get('/:id/poll/results', authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const votes = await dbAll('SELECT option_index, COUNT(*) as count FROM poll_votes WHERE post_id = ? GROUP BY option_index', [id]);
        const userVote = await dbGet('SELECT option_index FROM poll_votes WHERE post_id = ? AND user_id = ?', [id, req.user.userId]);
...
router.get('/trending/hashtags', optionalAuthenticate, async (req, res) => {
    try {
...
router.get('/hashtag/:hashtag', optionalAuthenticate, async (req, res) => {
    try {
