...
const gitAuth = async (req, res, next) => {
    try {
        ...
        if (deployTokenData) {
            // It's a deploy token
            const repository = await dbGet('SELECT * FROM repositories WHERE id = ?', [deployTokenData.repoId]);
            if (repository) {
                // Update last used
                ...
                req.deployToken = deployTokenData;
        ...
        try {
            ...
            if (decoded.userId) {
                ...
                const user = await dbGet('SELECT username, email, role FROM users WHERE id = ?', [decoded.userId]);
                if (user) {
                    ...
                    if (password) {
                        ...
                        if (user) {
                            ...
                            if (isValidPassword) {
                                ...
                                const newToken = jwt.sign({ userId: user.id, username: user.username, email: user.email }, JWT_SECRET, { expiresIn: '7d' });
...
router.get('/:owner/:repo.git/info/refs', gitAuth, async (req, res) => {
    try {
...
router.post('/:owner/:repo.git/git-upload-pack', gitAuth, express.raw({ type: '*/*', limit: '100mb' }), async (req, res) => {
    try {
        const { owner, repo } = req.params;
        const repository = await dbGet(`
      SELECT r.* FROM repositories r
...
router.post('/:owner/:repo.git/git-receive-pack', gitAuth, express.raw({ type: '*/*', limit: '100mb' }), async (req, res) => {
    try {
        const { owner, repo } = req.params;
        // Step 1: Always get user from database to ensure we have correct username and role
        const user = await dbGet('SELECT username, email, role FROM users WHERE id = ?', [req.user.userId]);
        if (!user) {
    ...
    `, [owner, repo]);
        ...
        if (!repository && owner === req.user.username) {
         ...
         VALUES (?, ?, ?, ?, ?, ?)`, [repoId, repo, '', req.user.userId, 0, 'main']);
      ...
      `, [repoId]);
        ...
        const isCollaborator = await dbGet('SELECT * FROM repository_collaborators WHERE repository_id = ? AND user_id = ?', [repository.id, req.user.userId]);
